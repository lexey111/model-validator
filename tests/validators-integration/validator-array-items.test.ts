...
describe('Validate array items', () => {
	test('should fail check array items', () => {
		...
		expect(result.errors['addresses[]'][0]).toBe('Length must be > 0');
	});
	...
	test('should fail check array items (2)', () => {
		...
		expect(result).toBeDefined();
		expect(result.level).toBe('error');
		expect(result.errors['addresses[]'][0]).toBe('Length must be > 2');
	});
	...
	test('should fail check array items per item (empty string)', () => {
		...
		expect(result).toBeDefined();
		expect(result.level).toBe('error');
		expect(result.errors['addresses[1]'][0]).toBe('Item must have the value');
	});
	...
	test('should fail check array items per item (undefined)', () => {
		...
		expect(result).toBeDefined();
		expect(result.level).toBe('error');
		expect(result.errors['addresses[2]'][0]).toBe('Item must have the value');
	});
